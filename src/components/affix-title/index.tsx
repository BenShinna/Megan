...
const AffixTitle = (props: AffixTitleProps) => {
  ...
  const blurClasses = classNames(
    ...
    'backdrop-blur-md bg-slate1/80', // 保留底色，配合遮罩实现渐变
    // 核心：使用 mask-image 实现从上到下的渐变消散
    // 顶部 40% 完全显示模糊，向下逐渐过渡到完全透明
    '[mask-image:linear-gradient(to_bottom,black_0%,black_40%,transparent_100%)]',
    '[-webkit-mask-image:linear-gradient(to_bottom,black_0%,black_40%,transparent_100%)]' // 兼容 Safari/iOS
  );
  ...
  useEffect(() => {
    handleScroll();
    // 优化：添加 passive: true 提升滚动性能，减少卡顿
