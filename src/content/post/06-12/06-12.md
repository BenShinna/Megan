...
*   **DJ音标**: /ɪnˈfɔːmd dɪˈsɪʒnz/
*   **解析**:
    *   **informed** /ɪnˈfɔːmd/: (adj.) 见多识广的；基于充分信息的；明智的。由动词 `inform` (通知) 加 `-ed` 构成。
    *   **decision** /dɪˈsɪʒn/: (n.) 决定。
*   **搭配**: `make informed decisions` 做出明智的决定；在充分了解情况的基础上做决定。
*   **语境**: ...helping consumers make **informed decisions** about food...（帮助消费者对食物做出明智的决定）。

**2. ultimately**
*   **DJ音标**: /ˈʌltɪmətli/
*   **解析**: (adv.) 最终；最后；根本地。
*   **同义词**: finally, eventually, in the end.
*   **语境**: ...and **ultimately** prevent unsafe consumption...（并最终防止不安全的食用）。这里表示最终的目的或结果。

**3. 2-date labeling system**
*   **DJ音标**: /ˌtuː ˈdeɪt ˈleɪbəlɪŋ ˈsɪstəm/
*   **解析**:
    *   **labeling** /ˈleɪbəlɪŋ/: (n.) 贴标签；标签系统。（英式拼写常为 `labelling`）。
*   **语境**: `the 2-date labeling system` 双日期标签系统。背景指美国常见的两种标签："BEST If Used By"（最佳食用日期，关乎质量）和 "USE By"（使用截止日期，关乎安全）。

**4. educational messaging that**
*   **DJ音标**: /ˌedʒuˈkeɪʃənl ˈmesɪdʒɪŋ ðæt/
*   **解析**:
    *   **educational** /ˌedʒuˈkeɪʃənl/: (adj.) 教育的，有教育意义的。
    *   **messaging** /ˈmesɪdʒɪŋ/: (n.) （通过广告、宣传等传递的）信息；消息传递。这里指教育性的宣传信息。
*   **语法**: `that explained the labels' meaning` 是定语从句，修饰 `messaging`。
*   **语境**: ...reading **educational messaging that** explained the labels' meaning.（阅读解释标签含义的教育信息）。

**5. respondents**
*   **DJ音标**: /rɪˈspɒndənts/
*   **解析**: (n.) （调查或问卷的）受访者；回答者。`respondent` 的复数形式。词根：`respond` (v. 回答) + `-ent` (表示人)。
*   **语境**: Less than half (46 percent) of study **respondents** knew...（不到一半的研究受访者知道……）。

**6. framing the messages with values**
*   **DJ音标**: /ˈfreɪmɪŋ ðə ˈmesɪdʒɪz wɪð ˈvæljuːz/
*   **解析**:
    *   **framing** /ˈfreɪmɪŋ/: (v.) `frame` 的现在分词。构建；表达；制定（框架）。在传播学中，"Framing" 指通过特定的角度或价值观来呈现信息，以影响受众的理解。
    *   **values** /ˈvæljuːz/: (n.) 价值观；重要性。
*   **语境**: `framing the messages with values` 用价值观来包装/构建信息。原文：Researchers explored if **framing the messages with values**, like saving money or avoiding waste...（研究者探索如果用价值观——如省钱或避免浪费——来包装信息，是否会影响效果）。

**7. another**
*   **DJ音标**: /əˈnʌðə/
*   **解析**: (pron.) 另一个。
*   **语境**: None of the seven value frames tested was significantly more effective at increasing understanding than **another**...（在测试的7种价值观框架中，没有哪一种比**另一种**显著更有效）。这里指这7种方法效果差不多。

**8. less attentive**
*   **DJ音标**: /les əˈtentɪv/
*   **解析**:
    *   **attentive** /əˈtentɪv/: (adj.) 注意的；留心的；关心的。搭配：`pay attention to` -> `be attentive to`。
*   **语境**: ...may make consumers **less attentive** to educational messaging...（可能导致消费者对教育信息**不那么留心/关注**）。
